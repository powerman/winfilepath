...
var globTests = []struct {
	...
	{"mat?h.go", "match.go"},
	{"*", "match.go"},
	{"../*/match.go", "../winfilepath/match.go"},
}

func TestGlob(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Glob tests require Windows filesystem behavior")
	}
...
func TestGlobSymlink(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("GlobSymlink tests require Windows filesystem behavior")
	}
...
func TestWindowsGlob(t *testing.T) {
	...
	for _, dir := range dirs {
		err := os.MkdirAll(Join(tmpDir, dir), 0o777)
		if err != nil {
			t.Fatal(err)
		}
	}
	for _, file := range files {
		err := os.WriteFile(Join(tmpDir, file), nil, 0o666)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestNonWindowsGlobEscape(t *testing.T) {
	t.Skip("winfilepath always uses Windows behavior")
}
