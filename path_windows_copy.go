...
import (
	...
	syscall "github.com/powerman/winfilepath/internal/winsyscall"
...
func join(elem []string) string {
	...
	for _, e := range elem {
		...
		case winos.IsPathSeparator(lastChar):
			...
			for len(e) > 0 && winos.IsPathSeparator(e[0]) {
				e = e[1:]
			}
			// If the path is \ and the next path element is ??,
			// add an extra .\ to create \.\?? rather than \??\
			// (a Root Local Device path).
			if b.Len() == 1 && strings.HasPrefix(e, "??") && (len(e) == len("??") || winos.IsPathSeparator(e[2])) {
				b.WriteString(`.\`)
			}
		case lastChar == ':':
